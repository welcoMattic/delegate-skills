...
Copilot did the typing; you own the judgment. Verify against reality, never the self-report, and
read the diff as generated code because a green gate cannot catch every failure mode.
...
Send anything blocking back to Copilot as a delta brief with `--resume-last`, or fix it in the
tree, and report either choice to the human. Run relevant guard skills if installed.

## Read-only runs: verify touchedFiles regardless

If you ran with `--read-only`, check `readOnlyViolation` in `result.json`. A `true` value means
the tree changed despite the tool denials. Tool denial is a model-level constraint, not a
kernel-enforced sandbox — always confirm `touchedFiles` and the diff, regardless of the flag used.
...
```bash
echo "The fix is right, but the test mocks the DB session. Use the real migrated fixture and
remove the unused import." | node "<skill-dir>/scripts/relay.mjs" --resume-last --cd /path/to/repo
```

Use `--resume-last` to continue the most recent session. Rework gets the same gate rerun, test
